 ...
 */
  ...
  */
  ...
  function newspiper_brightness( $hex, $steps ) {
 ...
 */
 ...
 function newspiper_hex_to_rgba( $hex, $alpha = false ) {
	$hex      = str_replace( '#', '', $hex );
	$length   = strlen( $hex );
 ...
 */
function newspiper_is_excerpt() {
	return get_theme_mod( 'post_archives_content', true );
}

function newspiper_has_breadcrumbs() {
...
function newspiper_has_secondary_menu() {
	return get_theme_mod( 'has_secondary_menu', false );
}

function newspiper_is_fixed_header() {
	return ( get_theme_mod( 'header-menu-position', 'fixed' ) == 'fixed' );
}

function newspiper_is_sticky_header() {
	return ( get_theme_mod( 'header-menu-position', 'sticky' ) == 'sticky' );
}

function newspiper_is_static_header() {
	return get_theme_mod( 'header-menu-position', 'sticky' ) == 'static';
}

function newspiper_has_multicolumn_layout() {
	return get_theme_mod( 'post_archives_columns', '1' ) !== '1';
}

function newspiper_default_mode(){
	return get_theme_mod( 'default_dark_mode', 0 ) ? 'dark' : 'light';
}

function newspiper_topmenu_has_wc_items() {
	return ( get_theme_mod( 'has_wc_icons', 1 ) == 1 && class_exists( 'WooCommerce' ) );
}

function newspiper_has_promo_banner() {
	return get_theme_mod( 'has_promo_banner', true );
}

function newspiper_has_categories_enabled( ) {
	return get_theme_mod( 'newspiper_categories_section_enable', true );
}

function newspiper_has_one_column_layout() {
	return get_theme_mod( 'post_archives_columns' , '1' ) == '1';
}
 ...
 */
 ...
 function newspiper_has_fullwidth_featured_image() {
 ...
 */
 ...
 function newspiper_is_page_fullwidth() {
	return get_theme_mod( 'page_layout', 'none' ) == 'none' && 'page' == get_post_type();
}

function newspiper_is_post_fullwidth() {
	return get_theme_mod( 'post_layout', 'right' ) == 'none' && is_single();
}

function newspiper_is_post_archives_fullwidth() {
	return get_theme_mod( 'post_layout', 'right' ) == 'none' && newspiper_is_blog();
}

function newspiper_is_shop_fullwidth() {
	if ( ! class_exists( 'Woocommerce' ) ) {
		return;
	}
	return get_theme_mod( 'shop_page_layout', 'right' ) == 'none' && is_shop();
}