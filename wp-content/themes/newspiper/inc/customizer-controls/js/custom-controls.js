jQuery( function( $ ) {
	...
	$( '.sortable_repeater_control' ).each( function() {
		...
		const defaultValuesArray = $( this ).find( '.customize-control-sortable-repeater' ).val().split( ',' );
		const numRepeaterItems = defaultValuesArray.length;

		if ( numRepeaterItems > 0 ) {
			// Add the first item to our existing input field
			$( this ).find( '.repeater-input' ).val( defaultValuesArray[ 0 ] );
			// Create a newspiper row for each newspiper value
	...
	// Append a newspiper row to our list of elements
	function newAppendRow( $element, defaultValue = '' ) {
		const newRow = '<div class="repeater" style="display:none"><input type="text" value="' + defaultValue + '" class="repeater-input" placeholder="https://" /><span class="dashicons dashicons-sort"></span><a class="customize-control-sortable-repeater-delete" href="#"><span class="dashicons dashicons-no-alt"></span></a></div>';
	...
	function newGetAllInputs( $element ) {
		const inputValues = $element.find( '.repeater-input' ).map( function() {
			return $( this ).val();
		} ).toArray();
	...
	$( '.customize-control-tinymce-editor' ).each( function() {
		...
		const tinyMCEToolbar1String = _wpCustomizeSettings.controls[ $( this ).attr( 'id' ) ].newtinymcetoolbar1;
		const tinyMCEToolbar2String = _wpCustomizeSettings.controls[ $( this ).attr( 'id' ) ].newtinymcetoolbar2;
		const tinyMCEMediaButtons = _wpCustomizeSettings.controls[ $( this ).attr( 'id' ) ].newmediabuttons;
 ...
 */
( function( $ ) {
	wp.customize.controlConstructor[ 'newspiper-range-slider' ] =
	wp.customize.Control.extend( {
		ready() {
			...
			let control = this,
				value,
				controlClass = '.customize-control-newspiper-range-slider',
			...
			$( controlClass + ' .newspiper-reset' ).on(
				'click',
				function() {
					let icon = $( this ),
						visible_area = icon
			...
			$( controlClass + ' .newspiper-range-slider-control' ).each(
				function() {
					const _this = $( this );
					_this
			...
			$( controlClass + ' .gp-device-controls > span' ).on(
				...
				function( event ) {
					...
					$(
						controlClass + ' .gp-range-slider-areas label'
					).each( function() {
						const _this = $( this );
						if ( device == _this.attr( 'data-option' ) ) {
							_this.show();
