...
export default function navigation() {
	...
	function toggleMobileMenu( e, $self ) {
		...
		document.addEventListener( 'click', function( e ) {
			...
			const tagNames = [ 'label', 'input', 'a', 'i', 'svg', 'circle', 'path', 'line', 'button', 'span' ];
	...
	function toggleFocus( e ) {
		if ( e.type === 'focus' || e.type === 'blur' ) {
			...
			while ( typeof self.className !== 'undefined' && -1 === self.className.indexOf( 'nav-menu' ) ) {
	...
	function mobileMenuFocusTrap( $self ) {
		...
		const focusableItems = header.querySelectorAll( '.menu-toggle, .menu-item > a, .social-icon > a, .site-header-cart > a, .search-icon > button, .dark-mode-widget' );
	...
	function searchModalFocusTrap() {
		...
		const focusableItems = this.querySelectorAll( 'input, .search-form button, .close' );
		const firstFocusableElement = focusableItems[ 0 ]; // get first element to be focused inside modal
		const lastFocusableElement = focusableItems[ focusableItems.length - 1 ]; // get last element to be focused inside modal
	...
	function mobileMenuFocusSkip() {
		...
		menuButtons[ 0 ].addEventListener( 'keydown', function( e ) {
			if ( this.getAttribute( 'aria-expanded' ) == 'false' && e.keyCode !== 13 ) {
