...
export function fixedMenu() {
	const fixedHeader = newspiper_customizer_object.fixed_header;
	const stickyHeader = newspiper_customizer_object.sticky_header;
	...
	const primaryMenu = document.getElementsByClassName( 'main-navigation-container' )[ 0 ];
	...
	if ( scrollBarPosition > 50 ) {
		...
		primaryMenu.className = primaryMenu.className.replace( ' fixed-header', '' );
		primaryMenu.style.position = newspiper_customizer_object.has_hero_image ? 'fixed' : 'static';
	}
	...
	if ( stickyHeader ) {
		if ( scrollBarPosition > 50 ) {
			...
			primaryMenu.className = primaryMenu.className.replace( 'sticky-header', '' );
		}
	}
}
...
export function stickyHeader() {
	if ( newspiper_customizer_object.sticky_header !== '1' ) {
		return;
	}
	...
	function toggleHeader( direction, curScroll ) {
		const header = document.getElementsByClassName( 'main-navigation-container' )[ 0 ];
