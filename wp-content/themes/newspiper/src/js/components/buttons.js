...
function inheritButtonBackgroundColor( button ) {
	if ( document.getElementsByClassName( 'p-btn-animation-border-move' ).length == 0 ) {
		return;
	}
	const bgColor = getComputedStyle( button ).backgroundColor == 'rgba(0, 0, 0, 0)' ? getComputedStyle( button ).borderColor : getComputedStyle( button ).backgroundColor;
