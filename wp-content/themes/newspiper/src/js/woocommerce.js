...
( function( $ ) {
	...
	function wrapButtons() {
		const buttons = document.querySelectorAll( '.newspiper-featured-products-wrapper .add_to_cart_button' );
		if ( buttons.length < 1 ) {
			return;
		}
	...
	$( 'body' ).on( 'wc_fragments_loaded added_to_cart', function( ) {
		...
		$( document ).ajaxSuccess( function( event, xhr, settings ) {
			if ( settings.url.indexOf( '?wc-ajax=add_to_cart' ) !== -1 ) {
				...
				$( this.activeElement ).next().addClass( 'add_to_cart_button wp-element-button' );
				wrapButtons();
			}
		} );
	} );
	...
	if ( document.getElementsByClassName( 'site-header-cart' ).length > 0 ) {
		document.getElementsByClassName( 'site-header-cart' )[ 0 ].addEventListener( 'mouseleave', unloadCartPreview );
	}
