...
class newspiper_notice {
	...
	private function is_dismiss_notice() {
		return apply_filters( 'newspiper_' . $this->name . '_notice_dismiss', true );
	}
	...
	public function get_notices() {
		...
		$academiathemes_theme_admin_notices = get_option( 'newspiper_admin_notices' );
	...
	public function hide_notices() {
		...
		if ( isset( $_GET['newspiper-hide-notice'] ) && isset( $_GET['_newspiper_notice_nonce'] ) ) {
			
			if ( ! wp_verify_nonce( wp_unslash( $_GET['_newspiper_notice_nonce'] ), 'newspiper_hide_notices_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'newspiper' ) );
			}

			if ( ! current_user_can( 'manage_options' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'newspiper' ) );
			}

			if ( $_GET['newspiper-hide-notice'] ) {
				
				$hide_notice_id = sanitize_text_field( wp_unslash( $_GET['newspiper-hide-notice'] ) );
				...
				if ( isset($run_update) ) {
					update_option( 'newspiper_admin_notices', $theme_admin_notices );
				}
