 ...
 * @since 1.0.0
 */
class Flash_News_Social_Icons_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			// widget ID.
			'flash_news_social_icons',
			// widget name.
			__( 'Ascendoor Social Icons', 'flash-news' ),
			// widget description.
			array( 'description' => __( 'Adds a social icon menu.', 'flash-news' ) )
		);
	}
	...
	public function form( $instance ) {
		...
		if ( ! $menus ) {
			...
			echo '<p>' . sprintf( __( 'No menus have been created yet. <a href="%s">Create some</a>.', 'flash-news' ), esc_url( admin_url( 'nav-menus.php' ) ) ) . '</p>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			return;
		}
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title', 'flash-news' ); ?></label>
			<input type="text" class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>"><?php esc_html_e( 'Select Menu:', 'flash-news' ); ?></label>
			<select id="<?php echo esc_attr( $this->get_field_id( 'nav_menu' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'nav_menu' ) ); ?>">
				<option value="0"><?php esc_html_e( '&mdash; Select &mdash;', 'flash-news' ); ?></option>
				<?php
